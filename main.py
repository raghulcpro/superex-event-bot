...
def get_formatted_date():
    return get_ist_now().strftime("%b %d")
...
def generate_report_text(target_date):
    ...
    for user, tasks in users.items():
        ...
        if analysis_links:
            ...
            for link in analysis_links: report_lines.append(f"Post Link - {link}")
        ...
        if signal_links:
            ...
            for i, link in enumerate(signal_links, 1): report_lines.append(f"Post link {i} - {link}")
            report_lines.append(date_str + "\n")
        ...
        if trade_links:
            ...
            for i in range(1, count + 1):
                ...
                if pts_add > 0: report_lines.append(f"#trade {i} → +{pts_add} points")
                else: report_lines.append(f"#trade {i} → 0 points (Slab Limit)")

            report_lines.append(f"Total Points for trade - {total_trade_pts}")
            for i, link in enumerate(trade_links, 1): report_lines.append(f"Post link {i} - {link}")
            report_lines.append("")
        ...
        if twitter_links:
            ...
            for link in twitter_links: report_lines.append(f"Post Link - {link}")
            report_lines.append(date_str + "\n")
